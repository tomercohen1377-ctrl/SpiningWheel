 ...
 *
 * Holds **one instance** of each dependency for the lifetime of the application
 * process — analogous to Hilt's `@Singleton` scope. Created lazily on first
 * access via [SpinWheelGraph.get(context)].
 *
 ...
 * the call sites (worker, widget, activity) stay identical.
 */
object SpinWheelGraph {
    ...
    class Graph(appContext: Context) {
        ...
        val repository: SpinWheelRepository =
            com.example.spinwheel.data.repository.SpinWheelRepositoryImpl(remote, local)
