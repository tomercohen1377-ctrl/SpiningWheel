...
/**
 * Parsed config extracted from the raw JSON loaded from the upstream URL.
 * Used by the use case that builds asset download URLs.
 */
data class SpinWheelConfig(
    val host: String,
    /** Total spin duration in ms — comes from `wheel.rotation.duration` in the JSON config. */
 ...
 * `provideGlance`) and the bridge process (JS calls into
 * `SpinWheelModule`). Without sharing, the widget re-decodes files from
 * disk on every tick instead of receiving an in-memory event.
 */
interface SpinWheelRepository {

    /**
     * Fetches the `wheel_config` JSON from [configUrl] via the remote data
     * source and persists it via the local data source. Returns the parsed
     * config or `null` on failure.
     */
    suspend fun fetchAndCacheConfig(configUrl: String): SpinWheelConfig?
