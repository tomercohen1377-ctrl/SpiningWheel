 ...
 *
 * The upstream `wheel_config` JSON ships this schema:
 * ```json
 * "host": "https://drive.google.com/drive/folders/FOLDER_ID",
 * "assets": { "bg": "bg.png", "wheel": "wheel.png", ... }
 * ```
 *
 * Drive doesn't serve `FOLDER_URL/filename` directly. We walk the folder HTML,
 * extract every file ID, HEAD each for its filename, and build
...
object DriveFileResolver {
    ...
    /**
     * Returns the direct download URL for [filename] in the folder identified
     * by [folderUrl]. **Must be called from a background thread.**
     *
     * - If [folderUrl] is a Drive folder URL → scrapes, resolves, caches.
     * - Otherwise → returns `folderUrl + filename` (CDN prefix).
     *
     * @throws IOException if [filename] cannot be resolved.
     */
